...
import com.sbvdeveloper.poc_nomina.service.PdfExportRetentionService;
...
public class PayrollController {
    ...
    private PdfExportRetentionService pdfExportRetentionService;
    ...
    public ResponseEntity<byte[]> generateReportRetention(@PathVariable() String type) throws IOException {
        ...
        byte[] report = type.equals("pdfNomina") ?
                pdfExportRetentionService.generateReport(type)
                : pdfExportSavingService.generateReport(type);

        //byte[] protectedPdf = pdfProtection.protectPdfWithPassword(report, "1234");
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_PDF);
        headers.setContentDispositionFormData("inline", type.equals("pdfNomina") ? "NominaRetención" + ".pdf" : "NominaAhorro" + ".pdf");
